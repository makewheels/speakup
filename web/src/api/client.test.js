...
describe("api/client 各方法 URL/method/body", () => {
  ...
  it("getPronunciationClip 带鉴权读取真实词片段", async () => {
  ...
  it("getSharedPronunciationClip 读取仍开启分享的词片段", async () => {
    const clip = new Blob(["clip"], { type: "audio/wav" });
    fetchMock.mockResolvedValue({ ok: true, blob: () => Promise.resolve(clip) });
    const result = await api.getSharedPronunciationClip("tok_abc", 0, 2);
    expect(callArgs()[0]).toBe("/api/share/tok_abc/attempts/0/pronunciation/issues/2/audio");
    expect(result).toBe(clip);
  });
