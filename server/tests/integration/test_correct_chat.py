...
def _make_attempt(client, user_id, practice_id):
    with _mock_correct():
        client.post(
            "/api/correct",
...
def test_chat_streams_and_persists(client, user_id, practice_id):
    _make_attempt(client, user_id, practice_id)
...
def test_chat_requires_existing_attempt(client, user_id, practice_id):
    with patch("routes.correct.followup_chat_stream", new=_fake_chat_stream):
        resp = client.post(
            "/api/correct/chat/stream",
...
def test_chat_rejects_other_users_practice(client, user_id, practice_id):
    _make_attempt(client, user_id, practice_id)
    other = client.post("/api/auth/login", json={"phone": "13900007777"}).json()["userId"]
    with patch("routes.correct.followup_chat_stream", new=_fake_chat_stream):
        resp = client.post(
            "/api/correct/chat/stream",
...
def test_chat_empty_question_400(client, user_id, practice_id):
    _make_attempt(client, user_id, practice_id)
    resp = client.post(
        "/api/correct/chat/stream",
