...
| AI 评估 | 主服务 `CHAT_*` + 两级 `CHAT_FALLBACK_1_*` / `CHAT_FALLBACK_2_*` | 火山优先，阿里云、DeepSeek 备用；故障自动切换，参数按各家接口生成 |
...
│       ├── ids.md           # ID 规范
│       ├── storage.md       # OSS 路径设计
│       └── 场景练习/        # scenario-mode / scenario-taxonomy / mixed-practice / scenario-evaluation
...
| 运营通知飞书应用凭据 / 功能邮件 | Infisical `speakup/prod/notifications` |
