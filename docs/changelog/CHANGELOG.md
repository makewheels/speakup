...
- **add(share)**：结果页直接分享本次结果，手机优先调系统分享面板，不支持时复制完整文案和链接；首次开启后取消或发送失败会撤回公开状态
- **change(corrector)**：纠正版与标准答案拆成并发的两个隔离请求；标准答案仅接收题目白名单快照，不包含用户原话、纠正或历史，两分支支持单路降级
- **change(web)**：中文结果标签统一为「你说的 / 纠正版 / 标准答案」；用户原话不分句且两端对齐；差距卡合并类型与标题，固定三行核心对比，移除 Meaning 展示，例句与翻译改为展开区；追问输入框加宽
