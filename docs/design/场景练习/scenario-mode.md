...
flowchart TD
    ...
    C --> D[POST /correct/stream<br/>deepseek-v4.1-flash 评估 SSE 流式]
    ...
    E -.后台.-> G[因材施教：取最弱 3 个表达<br/>deepseek-v4.1-flash 反向出题 → 定制题入库]
```
...
| 用途 | 模型 | 接口 | 说明 |
|------|------|------|------|
| 口语评估 / 定制出题 / 追问对话 | deepseek-v4.1-flash（火山方舟 Agent Plan） | OpenAI 兼容协议 (LangChain ChatOpenAI) | 当前生产配置；评估走 SSE 流式 |
