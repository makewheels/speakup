...
> 状态：首版代码已接入（2026-08-24），生产默认关闭；待开通腾讯服务、配置密钥并用真人录音验证后启用。
...
- 用户提交后**立即进入结果页**：先显示「逐句对照」中的原句与纠正版生成状态，不等完整 JSON；纠正完成后再上传录音并单独请求发音评测；
- 「你说的」与「纠正版」按 `sourceId` 一组一组上下排列；纠正版可比原句长、短或拆成两句，不做字符级强行对齐；
- 发音卡不阻塞文字结果；请求中显示紧凑生成态，失败时明确说明不影响文字纠正，未配置供应商时不发起请求；
- 独立标准答案与详细 gaps 默认折叠，控制结果页首屏长度；
...
| 阶段 | 内容 | 前置 | 产出 |
|---|---|---|---|
| P1 集成骨架 | 结果页先进入、逐句上下对照、折叠长内容、发音卡与持久化 | 无 | **已完成** |
| P2 腾讯适配 | WSS 签名、录音模式、两段式评测、字段归一化、feature flag | P1 | **代码已完成，默认关闭** |
| P3 体验启用 | 开通服务、配置生产密钥、真人录音验证后打开开关 | 账号开通+购买 | 待完成 |
| P4 验收（可选） | 教师标注试点，按调研 §7 门槛判 | 录音+标注 | 验收报告 |

## 12. 未决与后续

未决：① 腾讯服务开通与生产密钥；② 真人录音误报/延迟验证；③ 是否补完整音素教法规则库；④ 阿里云同批 A/B 后是否切回统一供应商。
