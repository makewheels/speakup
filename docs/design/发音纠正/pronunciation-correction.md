...
3. 一句人话解释（§2.3 归因链产出）；
4. 首版两个音频入口：「你的原音」（服务端按评测同源 WAV 时间轴即时裁片）与「标准音」（TTS）。浏览器不在完整 WebM/MP4 上 seek，避免容器时间轴和异步定位导致播放错词；即时片段不永久保存。「再练一次」必须等单词录音、模式 0 复测与状态持久化闭环一起上线，不先放无效按钮。
...
只存结构化结果与解释文案；**不新增原始音频副本或永久词片段**。回放时通过鉴权接口读取本轮原音，以 `startMs/endMs` 在与腾讯评测同源的规范化 WAV 上即时裁片并返回短 WAV。
